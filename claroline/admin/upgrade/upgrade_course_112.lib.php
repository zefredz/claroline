    ...
    if ( preg_match('/^1.10/',$currentCourseVersion) || preg_match('/^1.11/',$currentCourseVersion)  )
    {
        ...
        switch( $step = get_upgrade_status($tool,$course_code) )
        {
            case 1 :
                ...
                $sqlForUpdate[] = "ALTER TABLE   `" . $currentCourseDbNameGlu . "lp_learnPath`  ADD  `startDate` DATETIME NOT NULL DEFAULT 0 AFTER  `comment` ;";
            ...
            case 2 :
                // Add the field start date into lp_learnpath table
                $sqlForUpdate[] = "ALTER TABLE  `" . $currentCourseDbNameGlu . "lp_learnPath`  ADD  `endDate` DATETIME NOT NULL DEFAULT 0 AFTER  `comment` ;";
