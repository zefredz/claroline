...
{
    global $tbl_module;
    global $tbl_module_info;  
    global $tbl_dock;
    
    //find info about this module in DB
        
    $sql = "SELECT *,
                  D.`name` AS old_dockname,
                  D.`rank` AS old_rank
...
function install_module()
{
    ...
    {
        array_push($backlog_message, 'Problem with file upload'); 
    }
    else
    {
        array_push($backlog_message, "Temporary file is :".$_FILES['uploadedModule']['tmp_name']);
    }
    ...
    if ( preg_match("/.zip$/i", $_FILES['uploadedModule']['name']) && unzip_uploaded_file($_FILES['uploadedModule'], $uploaddir, $baseWorkDir, $maxFilledSpaceForModule,true))
    {
    ...
    $sql = "UPDATE `".$tbl_module."` as M
               SET M.`module_info_id` = '".$module_info_id."'
 ...
 *
 * @param the id of the module to uninstall
 * @return true if the uninstall process suceeded, false otherwise
 *
 */
 ...
 *
 * @param the dock from which we want this info
 * @return the max rank used for this dock
 */
...
function get_max_rank_in_dock($dockname)
{
    ...
    $max_rank = claro_sql_query_get_single_row($sql);
    $max_rank = $max_rank['mrank'];
    return (int)$max_rank; 
}
