...
if ( !empty($class_id) )
{
    switch ($cmd)
    {
        ...
        case 'unsubscribe_all' :
		    unregister_class_to_all_courses($class_id,$course_id);
    		break;
...
else
{
    ...
    if ( !empty($resultList) )
    {
        ...
        .  ' onclick="if (confirm(\'' . clean_str_for_javascript(get_lang('Unregister all courses ?')) . '\')){return true;}else{return false;}">'
        .    '<img src="' . get_path('imgRepositoryWeb') . 'unenroll.gif" alt="" /> '
        .    get_lang('Unregister class from all courses')
        .    '</a>';
    }
    else
    {
