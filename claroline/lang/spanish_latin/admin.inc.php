<?php // $Id$
/*
      +----------------------------------------------------------------------+
      | CLAROLINE version 1.4                                                |
      +----------------------------------------------------------------------+
      | Copyright (c) 2001, 2004, 2003 Universite catholique de Louvain (UCL)|
      +----------------------------------------------------------------------+
      |   Este programa es software libre; usted puede redistribuirlo y/o    | 
      |   modificarlo bajo los términos de la Licencia Pública General (GNU) | 
      |   como fué publicada por la Fundación de Sofware Libre; desde la     |
      |   versión 2 de esta Licencia o (a su opción) cualquier versión       |
      |   posterior.                                                         |
      |   Este programa es distribuído con la esperanza de que sea útil,     |
      |   pero SIN NINGUNA GARANTIA; sin ninguna garantía implícita de       |
      |   MERCATIBILILIDAD o ADECUACIÓN PARA PROPOSITOS PARTICULARES.        |
      |   Vea la Licencia Pública General GNU por más detalles.              |
      |   Usted pudo haber recibido una copia de la Licencia Pública         |
      |   General GNU junto con este programa; sino, escriba a la Fundación  |
      |   de Sofware Libre : Free Software Foundation, Inc., 59 Temple Place |
      |   - Suite 330, Boston, MA 02111-1307, USA. La licencia GNU GPL       |
      |   también está disponible a través de la world-wide-web en la        |
      |   dirección  http://www.gnu.org/copyleft/gpl.html                    |
      +----------------------------------------------------------------------+
      | Autores: Thomas Depraetere <depraetere@ipm.ucl.ac.be>                |
      |          Hugues Peeters    <peeters@ipm.ucl.ac.be>                   |
      |          Christophe Gesché <gesche@ipm.ucl.ac.be>                    |
      +----------------------------------------------------------------------+
      | Traducción :                                                         |
      |          Thomas Depraetere <depraetere@ipm.ucl.ac.be>                |
      |          Andrew Lynn       <Andrew.Lynn@strath.ac.uk>                |
      +----------------------------------------------------------------------+
      | Basado en la traducción al castellano de                             |
      |          Xavier Casassas Canals <xcc@ics.co.at>                      |
      | Adaptado al español latinoamericano en Agosto-2003 por               |
      |          Carlos Brys       <brys@fce.unam.edu.ar>                    |
      +----------------------------------------------------------------------+
 */

$langAddAdminInApache ="Agregar un ususrio administrador";
$langAddFaculties = "Agregar facultades"; 
$langSearchACourse  = "Buscar un curso";
$langSearchAUser  ="Buscar un usuario";

$langAdminBy = "Administrado por ";
$langAdministrationTools =" Herramientas de administración";
$langTools = "Herramientas";
$langState = "Estado"; 
$langDevAdmin = "Administración de Desarrollo";
$langLinksToClaroProjectSite ="Enlaces al sitio web del proyecto";
$langNomOutilTodo = "Administrar la lista cosas para hacer"; // to do
$langNomPageAdmin =\"Administración";
$langSysInfo  =\"Información acreca del Sistema";        // Show system status
$langCheckDatabase  = "Verificar la base de datos principal";        // Check Database
$langDiffTranslation = "Comparar traducciones"; // diff of translation
$langStatOf =\"Estadísticas de "; // Stats of...
$langSpeeSubscribe =\"Inscripción rápida como Verificador de Curso";
$langLogIdentLogout = "Registro de accesos";
$langLogIdentLogoutComplete = "Registro de accesos (extendido)"; 



// Stat
$langStatistiques =\"Estadísticas";


$langNbProf = "Cantidad de profesores";
$langNbStudents = "Cantidad de estudiantes";
$langNbLogin = "Cantidad de ingresos";
$langToday   ="Hoy";
$langLast7Days ="Ultimos 7 dias";
$langLast30Days ="Ultimos 30 dias";

$langNbAnnoucement = "Cantidad de Anuncios";

// Check Data base

$langPleaseCheckConfigForMainDataBaseName = "Por favor verifique estos valores
<br>
Nombre de la base de datos principal en 
<br>";
$langBaseFound ="Encontrado
<br>
Verificando las tablas de esta base";
$langNotNeeded = "no se necesita";
$langNeeded = "requerido";
$langArchive   ="archivo";
$langUsed      ="usado";
$langPresent   ="Correcto";
$langCreateMissingNow =\"¿Desea crear tablas ahora?";
$langCheckingCourses ="Verificando cursos ";
$langMissing   ="perdido";
$langExist     ="existe";

// Create Claro table
$langCreateClaroTables ="Crear Tablas para la base de datos principal";
$langTableStructureDontKnow ="La estructura de esta tabla es desconocida";

$langServerStatus ="Estado del servidor MySql : ";
$langDataBase = "Base de datos "; 
$langRun = "trabajo";
$langClient = "Cliente MySql ";
$langServer = "Servidor MySql ";
$langtitulary ="Titularidad";
$langLanguage ="Idioma";
$langUpgradeBase = "Actualizar la base de datos";

?>

