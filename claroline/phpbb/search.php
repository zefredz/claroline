...
<TR>
	...
	<TD BGCOLOR="<?php echo $color2?>" WIDTH="50%">
		...
		<?php
			$query = "SELECT forum_name,forum_id FROM `$tbl_forums` WHERE forum_type != 1";
			if(!$result = mysql_query($query,$db))
			{
				die("An Error Occured<<hr>phpBB was unable to query the forums database");
			}
...
$query = "SELECT u.user_id,f.forum_id, p.topic_id, u.username, p.post_time,t.topic_title,f.forum_name
			 FROM `$tbl_posts` p, `$tbl_posts_text` pt, `$tbl_users` u, `$tbl_forums` f, `$tbl_topics` t";
if(isset($term) && $term != "")
{
...
if(isset($search_username)&&$search_username!="")
{
   ...
   if(!$result = mysql_query("SELECT user_id FROM `$tbl_users`
                              WHERE username='$search_username'",$db))
	{
		error_die("An Error Occured<hr>phpBB was unable to query the forums database");
	}
...
//   $query .= " GROUP BY t.topic_id";
   ...
   $query .= " LIMIT 200";
	...
	if(!$result = mysql_query($query,$db))
	{
		die("An Error Occured<hr>phpBB was unable to query the forums database<BR>".mysql_error($db)."<BR>$query");
	}
