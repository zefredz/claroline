...
if($submit || $user_logged_in) {
   if($save) {
	 ...
	 setcookie($cookiename, $userdata['user_id'], $time, $cookiepath, $cookiedomain, $cookiesecure);
      ...
      include('page_header.php');
		...
		$sql = "UPDATE `$tbl_users`
	            SET user_viewemail='$viewemail', user_theme='$themes',
	                user_attachsig = '$sig', user_desmile = '$smile',
	                user_html = '$dishtml', user_bbcode = '$disbbcode',
	                user_lang = '$lang'
	 ...
	 $n = "CHECKED";
      ...
      if(w$userdata[user_attachsig] == 1) 
	$always_sig = "CHECKED";
      else
...
<?php
	$sql = "SELECT theme_id, theme_name FROM `$tbl_themes` ORDER BY theme_name";
	if(!$result = mysql_query($sql, $db))
