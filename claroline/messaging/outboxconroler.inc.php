 ...
 */
    ...
    $acceptedCmdList = array('rqSearch','exDelete','rqDelete');
    ...
    if (isset($_REQUEST['cmd']) && in_array($_REQUEST['cmd'], $acceptedCmdList))
    {
        ...
        if ($_REQUEST['cmd'] == 'exDelete' && isset($_REQUEST['messageId']))
        {
            if ( !claro_is_platform_admin() )
            {
                claro_die(get_lang('Not allowed'));
            }
            $message = new SentMessage($_REQUEST['messageId']);
            $message->delete();
        }
        
        if ($_REQUEST['cmd'] == 'rqDelete' && isset($_REQUEST['messageId']))
        {
            if ( !claro_is_platform_admin() )
            {
                claro_die(get_lang('Not allowed'));
