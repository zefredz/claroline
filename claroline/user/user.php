...
foreach ( $userList as $thisUser )
{
    ...
    // Display lastname 
    if ( $linkToUserInfo || $is_allowedToEdit )
    {
        echo '<a href="userInfo.php?uInfo='.$thisUser['user_id'].'">' . ucfirst(strtolower($thisUser['nom'])) . '</a>' ;
    }
    else
    {
    ...
    if ( $linkToUserInfo || $is_allowedToEdit )
    {
        echo '<a href="userInfo.php?uInfo='.$thisUser['user_id'].'">' . ucfirst(strtolower($thisUser['prenom'])) . '</a>';
    }
    else
    {
