...
ClaroBreadCrumbs::getInstance()->append(
    htmlspecialchars($wiki->getTitle()), htmlspecialchars(Url::Contextualize($_SERVER['PHP_SELF']
    ...
    case 'edit':
        {
            ...
            ClaroBreadCrumbs::getInstance()->append(
                htmlspecialchars($dispTitle), htmlspecialchars(Url::Contextualize('page.php?action=show&wikiId='
                        . $wikiId . '&title=' . $title))
            );
    ...
    case 'preview':
        {
            ...
            ClaroBreadCrumbs::getInstance()->append(
                htmlspecialchars($dispTitle), htmlspecialchars(Url::Contextualize(
    ...
    case 'recent':
        {
            ...
            $out .= '<p>'
                ...
                . ( $previous !== false ? ' ' . '<a href="' . $script . '&offset='
                    . $previous . '&step=' . (int) $step . '">&lt; ' . get_lang('Previous') . '</a>' : ' &lt; Previous' )
                . ' ' . '<a href="' . $script . '&offset=0&step=0">' . get_lang('All') . '</a>'
                . ( $next !== false ? ' ' . '<a href="' . $script . '&offset='
                    . $next . '&step=' . (int) $step . '">' . get_lang('Next') . ' &gt;</a>' : ' Next &gt;' )
                . ' ' . '<a href="' . $script . '&offset='
            ...
            if (is_array($recentChanges))
            {
                ...
                {
                    $pgtitle = ( '__MainPage__' == $recentChange['title'] ) ? get_lang("Main page") : $recentChange['title']
                    ;
    ...
    case 'history':
        {
            ...
            $out .= '<p>'
                . '<a href="' . htmlspecialchars($script . '&offset='
                    . $first . '&step=' . (int) $step) . '">&lt;&lt; '
                . get_lang('First') . '</a>'
                . ( $previous !== false ? ' ' . '<a href="' . htmlspecialchars($script . '&offset='
                        . $previous . '&step=' . (int) $step) . '">&lt; '
                    . get_lang('Previous') . '</a>' : ' &lt; ' . get_lang('Previous') )
                . ' ' . '<a href="' . htmlspecialchars($script . '&offset=0&step=0') . '">'
                . get_lang('All') . '</a>'
                . ( $next !== false ? ' ' . '<a href="' . htmlspecialchars($script . '&offset='
                        . $next . '&step=' . (int) $step) . '">'
                    . get_lang('Next') . ' &gt;</a>' : get_lang('Next') . ' &gt;' )
                . ' ' . '<a href="' . htmlspecialchars($script . '&offset='
    ...
    case 'rqSearch':
        {
            $searchForm = '<form method="post" action="'
                ...
                . claro_html_button(
                    htmlspecialchars(Url::Contextualize(
                            $_SERVER['PHP_SELF'] . '?wikiId=' . $wikiId)), get_lang("Cancel"))
                . '</form>' . "\n"
            ;
