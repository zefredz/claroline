...
&nbsp;&nbsp;
<small><a href="../user/user.php"><?php echo $langAddTutors ?></a></small>
<td>
<label for="maxMember">
<?php
...
while ($myMember = mysql_fetch_array($resultMember))
{
    ...
    .    ucwords(strtolower($myMember['name'] . ' ' . $myMember['firstname']))
    .    '</option>'."\n"
    ;
}
...
while ($myNotMember = mysql_fetch_array($resultNotMember))
{
    ...
    .    ucwords(strtolower($myNotMember['name'] . ' ' . $myNotMember['firstname']))
    ;
