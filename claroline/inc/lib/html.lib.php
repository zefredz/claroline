...
function claro_parse_user_text($userText)
{
    ...
    if( !preg_match('/<!-- content:[^(\-\->)]*-->/', $userText)
    	&& !preg_match('/<br( \/)?>/i', $userText)
    	&& !preg_match('/<(p|div|ul|ol|dl|blockquote|table|pre|h\d)[^>]?>/i', $userText)
    )
    {
        // if we do not find line break nor block element tags nor content metadata tag
        // Note the '<!-- content: ... -->' should be introduced by editor
