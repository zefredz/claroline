    ...
    */
    ...
    {  
        ...
        if (($_courseTool['label']=="CLDOC___") && ($fileList['type'][$fileKey] == A_DIRECTORY))
        {
            ...
            {
                $ressource_identification = $_cid.":".$_tid.":".$_gid.":".$ressource['ressource_id'].":".$ressource['date'];
                $pattern = '/' . $_cid.":".$_tid.":".$_gid.":".preg_quote($ressourceId, '/').".*:".$ressource['date'] . '/';
        ...
        if (isset($this->toolNotifiedRessourceList[$ressourceId])
             && !isset($_SESSION['ConsultedRessourceList'][$_cid.":".$_tid.":".$_gid.":".$ressourceId.":".$this->toolNotifiedRessourceList[$ressourceId]['date']]))
                   
        {
            //now, the ressource is seen 
            
            if ($setAsViewed) $_SESSION['ConsultedRessourceList'][$_cid.":".$_tid.":".$_gid.":".$ressourceId.":".$this->toolNotifiedRessourceList[$ressourceId]['date']] = TRUE;      
            return true;
        }
        else
        {
    ...
    {  
        ...
        foreach ($notified_ressources as $ressource)
        {
            ...
            $pattern = '/' . $_cid.":".$_tid.":".$_gid.":".$forumId."-.*:".$ressource['date'] . '/';
    ...
    function get_notified_ressources($course_id, $date, $user_id, $gid = "0", $tid)
    {
        ...
        if ( !isset($_SESSION['firstLogin']) || !$_SESSION['firstLogin'] ) 
        {
            ...
            foreach($ressourceList as $ressource)
            {
                $ressources[$ressource['ressource_id']] = $ressource;
            }
            return $ressources;    
        }
        else
    ...
    */
    ...
    function get_last_action_before_login_date($user_id)
    {
        ...
        { 
            ...
            $sql = "SELECT MAX(`access_date`) AS MAXDATE
                      FROM `".$tbl_c_names['track_e_access']."` AS STAT, `".$tbl_rel_course_user."` AS CU
            ...
            if (isset($result[0]['MAXDATE'])) 
            {
                if (strtotime($result[0]['MAXDATE']) > strtotime($last_action_date)) 
                {
                    $last_action_date = $result[0]['MAXDATE'];
                }
            }                  
        }                   
                                   
            //return (and save in session) the latest action : last login or latest action in the tracking tables 
