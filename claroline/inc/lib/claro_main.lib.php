...
function claro_get_restricted_courses ($categoryId, $userId)
{
    ...
    if (!is_null($categoryId))
        $sql .= "
            ...
            LEFT JOIN `" . $tbl_rel_course_category . "` AS rcc 
            ON (c.cours_id = rcc.courseId AND c.sourceCourseId IS NULL)";
    ...
    if (!is_null($categoryId))
        $sql .= "
            AND ( rcc.categoryId = " . (int) $categoryId . " OR c.sourceCourseId IS NOT NULL)";
    
    $sql .= "
            ORDER BY UPPER(c.administrativeNumber), c.intitule";
