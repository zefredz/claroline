                             ...
                             )
{
    ...
    GLOBAL $coursesRepositorySys,$prefixAntiNumber,$prefixAntiEmpty,$nbCharFinalSuffix,$DEBUG,$singleDbEnabled;
    ...
    while (!$keysAreUnique)
    {
        ...
        if (file_exists($coursesRepositorySys . '/' . $keysCourseRepository))
        {
            ...
            if ($DEBUG) echo '[dir'.$coursesRepositorySys . '/' . $keysCourseRepository.']<br>';
        };
...
// "bb_posts_text";
    ...
    $sqlList[] = "CREATE TABLE `".$TABLEPHPBBPOSTSTEXT."` (
...
VALUES (NULL, '1', '0', '1', '1', '1', '1')");
    claro_sql_query("INSERT
                        ...
                        VALUES ( 1
                               ...
                               ,2,1,1,1,1,2,0,1)");
    claro_sql_query("INSERT INTO `".$TABLEPHPBBPOSTS."` VALUES
(1,1,1,1,NOW(),'127.0.0.1',\"".addslashes($nom)."\",\"".addslashes($prenom)."\")
");
    claro_sql_query("INSERT INTO `".$TABLEPHPBBPOSTSTEXT."` VALUES ('1',
'".addslashes($langMessage)."')");
...
############################## FORUMS  #######################################
    claro_sql_query("INSERT INTO `".$TABLEPHPBBTOPICS."` VALUES (1,'".addslashes($langExMessage)."',-1,NOW(),1,0,1,1,'0','1', '".addslashes($nom)."', '".addslashes($prenom)."')");
...
function register_course($courseSysCode, $courseScreenCode, $courseRepository, $courseDbName, $titular, $email, $faculte, $intitule, $languageCourse='', $uidCreator, $visibility, $registrationAllowed, $enrollmentKey='', $expirationDate='', $extLinkName='', $extLinkUrl='')
{
    ...
    $currenVersionFilePath = dirname(__FILE__) . '/../currentVersion.inc.php';
