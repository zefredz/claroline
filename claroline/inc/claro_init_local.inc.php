...
else
{
    ...
    if ( $login && $password ) // $login && $password are given to log in
    {
        ...
        if ( ! isset($claro_authUsernameCaseSensitive) ) $claro_authUsernameCaseSensitive = true ;
...
if (isset($_cid) && $_courseTool['label'])
{
    ...
    if (file_exists(dirname(__FILE__) . '/conf/' . $config_code . '.conf.php'))
        require dirname(__FILE__) . '/conf/' . $config_code . '.conf.php';
