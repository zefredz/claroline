...
{
    $clarolineBannerOutput .= claro_disp_debug_banner() ;
}
...
if( claro_is_in_a_course() || isset($nameTools) || ( isset($interbredcrump) && is_array($interbredcrump) ) )
{
    ...
    {
        $loginSourceUrl = (isset($_SERVER['HTTPS']) && ($_SERVER['HTTPS']=='on'||$_SERVER['HTTPS']==1) ? 'https://' : 'http://') 
                       . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];

        $clarolineBannerOutput .= "\n".'<div id="toolViewOption" style="padding-right:10px">'
            .'<a href="'.$clarolineRepositoryWeb.'auth/login.php'
            .'?sourceUrl='.urlencode(base64_encode($loginSourceUrl)). '" target="_top">'
            .get_lang('Login')
            .'</a>'
            .'</div>'."\n";
    }
