...
if ($_gid && $is_groupAllowed)
{
    ...
    $interbredcrump[]  = array ('url'=>'../group/group.php', 'name'=> $langGroupManagement);
...
if($is_allowedToEdit) // Document edition are reserved to certain people
{
	...
	if ($cmd == 'exUpload')
	{
		...
		{
		    $uploadedFile = $_REQUEST['cwd'].'/'.stripslashes($HTTP_POST_FILES['userFile']['name']);
            $imgFilePath = search_img_from_html($baseWorkDir.$uploadedFile);
    ...
    if ($cmd == 'exMkDir')
	{
		...
		{
			mkdir($baseWorkDir.$_REQUEST['cwd'].'/'.$newDirName, 0777);
			$dialogBox = $langDirCr;
		}
	}
...
// tracking this is here because we need  $nameTools
