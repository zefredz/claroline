...
if($submit || ($user_logged_in==1 && $mode=='viewip')) {
   ...
   switch($mode) {
    case 'del':
      ...
      $sql = "SELECT poster_id, post_id FROM `$tbl_posts` WHERE topic_id = '$topic'";
      if(!$r = mysql_query($sql, $db))
			die("Error - Could not query the posts database!");
      while($row = mysql_fetch_array($r)) {
	 		if($row[poster_id] != -1) {
	    		$sql = "UPDATE `$tbl_users` SET user_posts = user_posts - 1 
		...
		$sql = "SELECT post_id FROM `$tbl_posts` WHERE topic_id = '$topic'";
		if(!$r = mysql_query($sql, $db))
			die("Error - Could not query the posts database!");
      while($row = mysql_fetch_array($r))
      {
      ...
      $sql = "DELETE FROM `$tbl_posts` WHERE topic_id = '$topic'";
      if(!$result = mysql_query($sql, $db))
			die("Error - Could not remove posts from the database!");
      $sql = "DELETE FROM `$tbl_topics` WHERE topic_id = '$topic'";
      if(!$result = mysql_query($sql, $db))
			die("Error - Could not remove posts from the database!");
		$sql = "DELETE FROM $tbl_posts_text WHERE ";
      ...
      break;
    case 'move':
      $sql = "UPDATE `$tbl_topics` SET forum_id = '$newforum' WHERE topic_id = '$topic'";
      if(!$r = mysql_query($sql, $db))
			die("Error - Could not move selected topic to selected forum. Please go back and try again.");
      $sql = "UPDATE `$tbl_posts` SET forum_id = '$newforum' WHERE topic_id = '$topic'";
      if(!$r = mysql_query($sql, $db))
			die("Error - Could not move selected topic to selected forum. Please go back and try again.");
      ...
      break;
    case 'lock':
      $sql = "UPDATE `$tbl_topics` SET topic_status = 1 WHERE topic_id = '$topic'";
      if(!$r = mysql_query($sql, $db))
      ...
      break;
    case 'unlock':
      $sql = "UPDATE `$tbl_topics` SET topic_status = '0' WHERE topic_id = '$topic'";
      if(!$r = mysql_query($sql, $db))
	die("Error - Could not unlock the selected topic. Please go back and try again.");
      ...
      break;
    case 'viewip':
      $sql = "SELECT u.username, p.poster_ip FROM `$tbl_users` u, `$tbl_posts` p WHERE p.post_id = '$post' AND u.user_id = p.poster_id";
      if(!$r = mysql_query($sql, $db))
...
<?php
	$sql = "SELECT user_id, username, count(*) as postcount 
            FROM `$tbl_posts` p, `$tbl_users` u 
			WHERE poster_ip='".$m[poster_ip]."' && p.poster_id = u.user_id 
			GROUP BY user_id";
	if(!$r = mysql_query($sql, $db))
	{
