...
{
    claro_redirect($_SERVER['PHP_SELF'] . '?authModeReq=CAS&sourceUrl='.urlencode(base64_encode($sourceUrl)));
}

if ( $sourceUrl )
{
    $sourceUrlFormField = '<input type="hidden" name="sourceUrl" value="'.htmlspecialchars(base64_encode($sourceUrl)).'">';
}
else
{
    $sourceUrlFormField = '';
...
if ( ! claro_is_user_authenticated() && $uidRequired )
{
    ...
    if (get_conf('claro_CasEnabled',false))
    {
        ...
        .    '<a href="login.php?'. ($sourceUrl ? 'sourceUrl='.urlencode(base64_encode($sourceUrl)) : '').'&amp;authModeReq=CAS">'
        .    $login_text
        .    '</a>'
        .    '</div>';
...
else // LOGIN SUCCEEDED
{
    ...
    {
        // if previous page is the "lost password page" redirect to home page
        if ( strpos($sourceUrl,'lostPassword.php') ) $sourceUrl = get_path('url').'/index.php';
    ...
    elseif ( claro_is_in_a_course() )
    {
        ...
        claro_redirect(get_path('url') . '/claroline/course/index.php?cid=' . htmlspecialchars(claro_get_current_course_id()));
    }
    else
    {
        // redirect to claroline home page
