...
class Assignment
{
     ...
     * @author Sebastien Piraux <pir@cerdecam.be>
     * @return array
     * @TODO get the full list if authId is not specified (submissions and feedback for all authors)
     */
    function getSubmissionList($authId = '')
    {
    	...
    	elseif( $this->assignmentType == 'GROUP' )
    	{
    		$authCondition = '`group_id` = '.(int) $authId . ' AND ';
    	}
    	else
    	{
    		$authCondition = '`user_id` = '.(int) $authId . ' AND ';
    	}

        $sql = "SELECT `id`
                     FROM `" . $this->tblSubmission . "`
                    WHERE ".$authCondition." `assignment_id` = ". (int) $this->id;
